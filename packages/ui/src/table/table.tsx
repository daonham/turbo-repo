...
import { Dispatch, MouseEvent, PropsWithChildren, SetStateAction, useEffect, useState } from 'react';
...
type UseTableProps<T> = {
  ...
  cellRight?: (cell: Cell<T, any>) => React.ReactNode;
...
export function useTable<T extends any>(props: UseTableProps<T>): TableProps<T> & { table: TableType<T> } {
  const { data, rowCount, columns, defaultColumn, columnPinning, pagination, onPaginationChange } = props;
                      ...
                      >
                        <div className="flex items-center justify-between gap-6 !pr-0">
                          ...
                          >
                            ...
                            {isSortableColumn && <SortOrder order={sortBy === header.column.id ? sortOrder || 'desc' : null} />}
                          </ButtonOrDiv>
                        </div>
                      </th>
                    );
        ...
        <div className="sticky bottom-0 mx-auto -mt-px flex w-full max-w-full items-center justify-between rounded-b-[inherit] border-t border-gray-200 bg-white px-4 py-3.5 text-sm leading-6 text-gray-600">
          <div>
            ...
            <span className="font-medium">
              ...
              {Math.min((pagination.pageIndex - 1) * pagination.pageSize + pagination.pageSize, table.getRowCount())}
            </span>{' '}
